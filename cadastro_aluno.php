...
// VERIFICAR E POPULAR BANCO AUTOMATICAMENTE
function popularAlunosAutomaticamente($conexao) {
    // Verificar se já tem muitos alunos
    $check = mysqli_query($conexao, "SELECT COUNT(*) as total FROM alunos");
    $row = mysqli_fetch_assoc($check);
    
    if ($row['total'] < 10) { // Se tiver menos de 10 alunos, popular
        echo "<div class='alert alert-info mb-3'>
                <i class='fas fa-database me-2'></i>
                <strong>Populando banco de dados...</strong> Aguarde um momento.
              </div>";
        ...
        $ruas = ['Rua das Flores', 'Avenida Brasil', 'Rua XV de Novembro', 'Avenida Paulista'];
        $bairros = ['Centro', 'Vila Nova', 'Jardins', 'Bela Vista'];
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            $sql = "INSERT INTO alunos (nome_completo, data_nascimento, rua, numero, bairro, cep, nome_responsavel, tipo_responsavel, curso) 
                    VALUES ('$nome', '$data_nasc', '$rua', '$numero', '$bairro', '$cep', '$responsavel', '$tipo', '$curso')";
        ...
        echo "<div class='alert alert-success mb-3'>
                ...
                <strong>Banco populado com sucesso!</strong> 100 alunos foram inseridos automaticamente.
              </div>";
    }
}

// Chamar a função para popular
popularAlunosAutomaticamente($conexao);

// Código original do formulário continua abaixo...
?>
